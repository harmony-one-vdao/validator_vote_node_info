blacklist = ("@colorreader on telegram.",)

# Use one address as some names use chars that might not be parsed correctly..
updated_but_vers_not_found = (
    # "one1rw02h0xmks5ungx63tkdw0xn8xc4hjzf3xt8tw",  # PERMANENT Harmony Hive - Mech Warrior | Little man - lost wallet.
    # "one1kf42rl6yg2avkjsu34ch2jn8yjs64ycn4n9wdj",  # PERMANENT Pops - SOPH will ensure.
    # "one1nf50jmyvckd0795mu4qhm6zfxppesulafejzvl",  # CoinChowder
    # "one1nef0c8kgy30delcckf87jaegdp7s7ntrknvajj",  # Unifi Protocol Dao,
    # "one1s269hx6ch3u6hxuag5sp5ggwc68vem6d8du6ut",  # One United Power
    # "one1frsph4jajyp9qnf6nw4eft70v0gula7tptmurd",  # Rhythm
    # "one1kp4h9f9hj859vasam9wk037r0gnsmguq5sw3xd",  # Freyala
    # "one1mlkylwnsgsam8cdxzn05hal3ytjngsunlpmp2j",  # Peace Love Validator
    # "one1dc6ejsxvuldaz3zy5yvwk6dhuk50ke4xn6g5k3",  # Eric One
    # "one17cgf2kn2trxgpascgevf0e3ttymy90u3894r2n",  # Halcyon
    # "one102lcjqy44ett8wu07dxdtce6gm988j0eu3z6cy",  # Tilly One
    # "one1twcvkx63304dplxcmx0j9jm32u6er3k0hxuf7z",  # ONECelestial Validator
    # "one1txaatrq6cvm34gdgwegrzu97mrl7herh36m6yn",  # Crypt0Tech
    # "one12szkssdcwttafxtayvq3l07d6ktaz43xasmsda",  # HoundONE Validator\
    # "one16qtepesycmjdz707zzgy4a39z5r6er9ewz657p",  # iTower.ONE
    # "one1j6x78a450fm0quj6ejr9nfzrye55a877g2952r",  # Niko.One
    # "one1nt6c38fk2me9a62c0098edej8gl8csa0nfgd2m",  # Hodl Gvng
    # "one1qm52lvursg036ly7zmmk4wstpfvnawncs45zvm",  # CheekyOne
    # "one1a2rxp80kecp02335ls854mhjcm494hzs9fwa9w",  # GAMETOKEN - Play the game, earn crypto
)
